...
def _hydrate_review_sheet_item_meta(
    ...
    fallback_name = _review_sheet_fallback_us_product_name(result)
    current_name = str(result.get("us_product_name") or "").strip()
    raw_name = str(result.get("raw_name") or result.get("name") or "").strip()
    if not current_name or (raw_name and current_name.casefold() == raw_name.casefold()):
        result["us_product_name"] = fallback_name
    return result
...
def _review_sheet_fallback_us_product_name(item: dict[str, Any]) -> str:
    for key in ("normalized_name_candidate", "clean_name", "name", "raw_name"):
        ...
        if value:
            ...
            normalized = re.sub(r"(?:\s+[A-Za-zА-Яа-яЁё]/[A-Za-zА-Яа-яЁё])+$", "", normalized)
...
def _invoice_register_item_row(
    ...
    if item is None:
        ...
        unit_in_us = row_meta.get("us_unit") or row_meta.get("accounting_unit_candidate") or unit
        quantity_in_us = (
            row_meta.get("quantity_us")
            if row_meta.get("quantity_us") is not None
            else row_meta.get("accounting_quantity_candidate")
        )
        if quantity_in_us is None:
            quantity_in_us = quantity
        price_in_us = row_meta.get("price_us") if row_meta.get("price_us") is not None else price
        ordered_quantity = ""
        price_by_pricelist = ""
        deviation_from_pricelist = ""
        upload_to_us = ""
        ...
        units_per_package = row_meta.get("units_per_package", "")
...
def create_real_google_sheet_for_review(db: Session, receiving: Receiving, public_api_base_url: str | None = None) -> dict:
    existing_result = _existing_google_sheet_export_result(db, receiving.id)
...
def _existing_google_sheet_export_result(
    db: Session, receiving_id: int
) -> dict[str, Any] | None:
    export = (
        db.query(AccountingExport)
        .filter(
            AccountingExport.receiving_id == receiving_id,
            AccountingExport.target_system == "google_sheets",
            AccountingExport.status.in_(("spreadsheet_created", "spreadsheet_updated")),
        )
    ...
    if export is None:
        return None
    try:
        result = json.loads(export.payload_json or "{}")
    except json.JSONDecodeError:
        return None
    if not isinstance(result, dict) or not result.get("spreadsheet_id"):
        return None
    result = dict(result)
    result["mode"] = "already_exported"
    return result
